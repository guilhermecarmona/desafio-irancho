<template>
  <div>
    <section class="mb-4">
      <div class="row">
        <h2 class="col-md-4 mb-5">Pessoa</h2>
        <router-link to="pessoa/criar" class="col-md-3 my-3">+ Adicionar</router-link>
      </div>
      <div v-if="!dados || dados.length > 0" class="row">
        <div v-for="d in dados" class="col-md-5 mr-4 mb-4 card-custom">
          ...
          <p class="text-right mb-0">
            <router-link :to="{name: 'editar-pessoa', params: {id:d.id}}" class="text-primary" v-on:click="onEdit(d)">Editar</router-link>
            &nbsp;&nbsp;/&nbsp;&nbsp;
    ...
    <Modal v-bind:type="'pessoa'" v-bind:modalShow="modalShow" 
      v-on:delete-pessoa="onDeleteConfirm" v-on:cancel-delete-pessoa="modalShow = false"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    getData: function (){
      return axios.get(`${config.urlRest}/pessoa`)
      .then((resp) => {
    ...
    onDeleteConfirm(){
      return axios.delete(`${config.urlRest}/pessoa/${this.id}`)
        .then(() => {
          this.getData();
        })
        .catch(e => {
