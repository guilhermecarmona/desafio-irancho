<template>
  <div>
    <section class="mb-4">
      <div class="row">
        <h2 class="col-md-4 mb-5">Animal</h2>
        <router-link to="animal/criar" class="col-md-3 my-3">+ Adicionar</router-link>
      </div>
      <div v-if="!dados || dados.length > 0" class="row">
        <div v-for="d in dados" class="col-md-5 mr-4 mb-4 card-custom">
          ...
          <p class="text-right mb-0">
            <router-link :to="{name: 'editar-animal', params: {id:d.id}}" class="text-primary" v-on:click="onEdit(d)">Editar</router-link>
            &nbsp;&nbsp;/&nbsp;&nbsp;
    ...
    <Modal v-bind:type="'animal'" v-bind:modalShow="modalShow" 
      v-on:delete-animal="onDeleteConfirm" v-on:cancel-delete-animal="modalShow = false"/>
  </div>
</template>
...
export default {
  ...
  methods: {
    getData: async function (){
      return axios.get(`${config.urlRest}/animal`)
      .then((resp) => {
        this.dados = resp.data.map(dado => {
          ...
          for(const field of Object.keys(dado)){
            if(field.includes('dt_')) {
              ...
              object[field] = dado[field].no_pessoa;
            }else{
    ...
    onDeleteConfirm(){
      return axios.delete(`${config.urlRest}/animal/${this.id}`)
        .then(() => {
          this.getData();
        })
        .catch(e => {
